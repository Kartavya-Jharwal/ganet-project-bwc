...
if header_match:
    ...
    # Remove existing scattered downloads from index.html header
    header = re.sub(r'<div class="cta-group".*?</div>', '', header, flags=re.DOTALL)
    
    header = re.sub(r'</header>', r'''
            <div class="mt-loose" style="margin-top:var(--spacing-6); display:flex; gap:1rem;">
                <a href="./deliverables/source/Client_Post_Mortem_Investment_Challenge-Team-5-BWC_CH200.pptx" class="cta-btn cta-primary" download>Download Post-Mortem Presentation</a>
                <a href="./deliverables/source/Final%20Excel%20model.xlsx" class="cta-btn cta-secondary" download>Download Excel Trading Model</a>
            </div>
        </header>''', header)
...
story_content = re.sub(r'<div class="cta-group".*?</div>', '', story_content, flags=re.DOTALL)
story_content = re.sub(r'<section class="narrative-section fade-up artifacts-hero">.*?</section>', '', story_content, flags=re.DOTALL)
...
clean_deliv = re.sub(r'<div class="carousel-scaffold">.*?<div class="carousel-controls">.*?</div>', r'''
                  <div class="text-body text-muted mb-tight" style="padding:1rem 0;">
                      <a href="./deliverables/source/Client_Post_Mortem_Investment_Challenge-Team-5-BWC_CH200.pptx" target="_blank" class="cta-btn cta-secondary" style="display:inline-block">OPEN POST-MORTEM PRESENTATION</a>
                  </div>
...
# Switch paths to be relative to root
...
final_html = re.sub(r'<main[^>]*>.*?</main>', lambda m: full_main, index_html, flags=re.DOTALL)

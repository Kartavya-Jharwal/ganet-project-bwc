...
def ingest(
    continuous: bool = typer.Option(False, "--continuous", "-c", help="Run ingestion on an APScheduler loop indefinitely."),
):
    """
    ...
    try:
        ...
        print_diagnostic_error(
            f"DuckDB Sync Failed. ({str(e)})", 
            "Make sure your Appwrite containers are deployed and `doppler run` injected secrets."
        )
        raise typer.Exit(1)

    with console.status("[bold cyan]Spinning up the Main Ingestion Pipeline...[/bold cyan]", spinner="dots4"):
...
def consume():
    ...
    try:
        from quant_monitor.main import main
        with console.status("[bold cyan]Executing Topographical Engine...[/bold cyan]", spinner="blue_pulse_math"):
            main()
        ...
        print_diagnostic_error(
            str(e), 
            "Data feed might be stale. Run `quant sync-data` first before consuming."
        )
...
def health():
    ...
    table.add_row("DuckDB Exists", "[bold green]YES[/bold green]" if os.path.exists("portfolio.duckdb") else "[bold red]MISSING[/bold red]")
    table.add_row("Cached Folders", "[bold green]YES[/bold green]" if os.path.exists(".cache") else "[bold yellow]EMPTY[/bold yellow]")
    
    doppler_active = "DOPPLER_CONFIG" in os.environ or "APPWRITE_API_KEY" in os.environ
    table.add_row("Doppler Secrets", "[bold green]INJECTED[/bold green]" if doppler_active else "[bold red]MISSING (Run under 'doppler run -- ')[/bold red]")
...
def dashboard(
    view: str = typer.Option(None, "--view", "-v", help="Select view: overview, signals, regime, montecarlo, health"),
